...
在配置文件（`.spaceflow/spaceflow.json`、`.spaceflowrc` 或兼容的 `spaceflow.json`）中通过 `support` 字段指定需要关联的编辑器：
...
3. 根据 `support` 配置，将资源关联到对应编辑器目录：
   - **skill** — 将文件**复制**到 `.cursor/skills/`、`.windsurf/skills/` 等目录
   - **command** — 生成 `.md` 命令文件到 `.cursor/commands/` 等目录
   - **mcp** — 将 MCP Server 配置写入 `.cursor/mcp.json` 等文件
...
使用 `-g` 参数时，Extension 安装到 `~/.spaceflow/node_modules/`，资源关联到家目录下的编辑器目录（如 `~/.cursor/skills/`）。

## 目录结构示例

配置 `support: ["agents", "cursor", "windsurf"]` 后，安装一个包含 `skill` 和 `command` 导出的 Extension 会生成如下结构：

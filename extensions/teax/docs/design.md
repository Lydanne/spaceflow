...
Teax 扩展路由（Gitea 无，采用 GitHub 风格）：
  ...
  /:orgName/:projectName/workspaces — 项目工作区列表（Teax 扩展）
...
通用路由：
  ...
  /-/admin/workspaces              — 全局工作区管理（Teax 扩展）
...
工作区（项目级，嵌套在项目下）：
  GET    /api/orgs/{orgId}/projects/{projectId}/workspaces             — 项目工作区列表
  POST   /api/orgs/{orgId}/projects/{projectId}/workspaces             — 创建工作区（启动容器）
  GET    /api/orgs/{orgId}/projects/{projectId}/workspaces/{workspaceId}         — 工作区详情
  DELETE /api/orgs/{orgId}/projects/{projectId}/workspaces/{workspaceId}         — 删除工作区（销毁容器 + 清理卷）
  POST   /api/orgs/{orgId}/projects/{projectId}/workspaces/{workspaceId}/start   — 启动已停止的工作区
  POST   /api/orgs/{orgId}/projects/{projectId}/workspaces/{workspaceId}/stop    — 停止工作区（保留数据卷）
...
管理员（UUID-based）：
  ...
  GET    /api/admin/workspaces                  — 全局工作区列表（所有组织、所有项目）
...
| **Pages** | 静态页面托管配置 |
| **Workspaces** | 工作区管理：创建/启动/停止/删除容器化开发环境，通过 Web VSCode 或 Remote SSH 访问 |
| **设置** | 通知配置、危险操作（删除项目） |
...
代理通过 Nitro server route + `httpProxy` 实现：
...
│
├── /workspace/:name                 # 工作区访问（Teax 扩展，/workspace/ 前缀避免路由冲突）
│   ├── /ide/**                      # Web VSCode（反向代理到容器内 openvscode-server）
│   └── /**                          # 测试环境预览（反向代理到容器内应用端口）
│
...
#### 6. Workspaces Tab `/:orgName/:projectName/workspaces`
...
│  │ ┌──────────────────────────────────────────────┐    │   │
│  │ │  my-feature-dev                       ● 运行中 │    │   │
│  │ │  分支: feature/login   创建者: zhangsan       │    │   │
│  │ │  创建于 2小时前                                │    │   │
│  │ │  [打开 IDE]  [预览]  [停止]  [删除]           │    │   │
│  │ └──────────────────────────────────────────────┘    │   │
│  │ ┌──────────────────────────────────────────────┐    │   │
│  │ │  bugfix-env                           ● 运行中 │    │   │
│  │ │  分支: fix/api-error   创建者: lisi           │    │   │
│  │ │  创建于 30分钟前                               │    │   │
│  │ │  [打开 IDE]  [预览]  [停止]  [删除]           │    │   │
│  │ └──────────────────────────────────────────────┘    │   │
│  └─────────────────────────────────────────────────────┘   │
...
│  │  名称            项目          创建者   状态   操作    │ │
│  │  ──────────────────────────────────────────────────── │ │
│  │  my-feature-dev  xg/nodecloud  张三    ● 运行中 [删除] │ │
│  │  bugfix-env      xg/webapp     李四    ● 运行中 [删除] │ │
│  │  old-experiment  xg/nodecloud  张三    ○ 已停止 [删除] │ │
│  └───────────────────────────────────────────────────────┘ │
│                                                             │
│  统计: 运行中 2 / 已停止 1 / 总计 3                          │
│                                                             │
...
#### 10. 个人账号 - 飞书绑定 `/account/feishu`（⏳ 未实现）
...
| `WorkspaceCard` | 工作区卡片：名称、分支、状态、操作按钮（打开 IDE/预览/停止/删除） |
| `WorkspaceCreateModal` | 创建工作区弹窗：名称、分支选择、资源配置 |
| `AdminWorkspaceList` | 管理员全局工作区列表：跨组织/项目查看和管理所有工作区 |
...
工作区是与仓库绑定的容器化开发环境，每个工作区对应一个 Docker 容器实例。

```typescript
interface Workspace {
  id: string;
  repository_id: string;           // 关联 Repository.id
  organization_id: string;         // 冗余字段，便于全局查询
  name: string;                    // 全局唯一名称（用于 URL 路径 /workspace/{name}/）
  status: WorkspaceStatus;
  branch: string;                  // Git 分支

  // 容器信息
  container_id?: string;           // Docker 容器 ID
  container_host: string;          // 容器宿主地址（默认 127.0.0.1）
  ide_port?: number;               // openvscode-server 映射到宿主的端口
  app_port?: number;               // 测试环境应用映射到宿主的端口
  image: string;                   // 容器镜像（默认 teax/workspace:latest）
  ...
  // 生命周期
  created_by: string;              // 创建者 User.id
  started_at?: Date;               // 最近启动时间
  stopped_at?: Date;               // 最近停止时间
...
interface WorkspaceResources {
  cpuCores: number;                // CPU 核数（默认 2）
  memoryMB: number;                // 内存 MB（默认 4096）
  storageMB?: number;              // 存储上限 MB（默认 10240）
}
```
...
│   ├── workspace/             # 工作区相关组件
│   │   ├── WorkspaceCard.vue      # 工作区卡片（名称、分支、状态、操作按钮）
│   │   ├── WorkspaceCreateModal.vue # 创建工作区弹窗（名称、分支、资源配置）
│   │   └── AdminWorkspaceList.vue # 管理员全局工作区列表
│   └── admin/                 # 管理面板组件
...
│   │       ├── workspaces.vue # Workspaces（Teax 扩展）
│   │       └── settings.vue   # 项目设置
...
│   │               └── workspaces/       # 工作区管理
│   │                   ├── index.get.ts      # 项目工作区列表
│   │                   ├── index.post.ts     # 创建工作区（启动容器）
│   │                   └── [workspaceId]/
│   │                       ├── index.get.ts      # 工作区详情
│   │                       ├── index.delete.ts   # 删除工作区（销毁容器 + 清理卷）
│   │                       ├── start.post.ts     # 启动已停止的工作区
│   │                       └── stop.post.ts      # 停止工作区（保留数据卷）
│   ├── permissions/           # 全局权限定义
...
│   │   └── workspaces/
│   │       ├── index.get.ts       # 全局工作区列表（所有组织、所有项目）
│   │       └── [workspaceId].delete.ts # 管理员强制删除工作区
│   └── webhooks/
│       └── gitea.post.ts          # Gitea Webhook 接收（HMAC-SHA256）
├── routes/                    # 非 API 路由（网关代理等）
│   └── workspace/
│       └── [name]/
│           └── [...path].ts       # 工作区网关代理（IDE + 测试环境，支持 WebSocket）
├── middleware/                # 服务端中间件
...
│   ├── workspace.service.ts   # 工作区 CRUD + 容器生命周期管理
│   └── container/             # 容器运行时抽象层
│       ├── container-runtime.ts   # ContainerRuntime 接口定义
│       └── docker-runtime.ts      # Docker 实现（dockerode）
├── shared/                    # 前后端共享代码
...
│   │   ├── workspace.dto.ts   # workspaces + createWorkspaceBody schema
...
CREATE TABLE workspaces (
  ...
  repository_id UUID REFERENCES repositories(id) ON DELETE CASCADE,
  organization_id UUID REFERENCES organizations(id) ON DELETE CASCADE,
  name VARCHAR(255) UNIQUE NOT NULL,   -- 全局唯一名称（URL 路径标识 /workspace/{name}/）
  status VARCHAR(50) DEFAULT 'creating', -- creating | running | stopping | stopped | starting | failed | deleting
  branch VARCHAR(255) NOT NULL,        -- Git 分支
  container_id VARCHAR(255),           -- Docker 容器 ID
  container_host VARCHAR(255) DEFAULT '127.0.0.1', -- 容器宿主地址
  ide_port INTEGER,                    -- openvscode-server 映射到宿主的端口
  app_port INTEGER,                    -- 测试环境应用映射到宿主的端口
  image VARCHAR(512) DEFAULT 'teax/workspace:latest', -- 容器镜像
  resources JSONB DEFAULT '{"cpuCores": 2, "memoryMB": 4096}', -- WorkspaceResources
  created_by UUID REFERENCES users(id),
  started_at TIMESTAMPTZ,
...
services:
  teax:
    ...
    ports:
      - "3000:3000"
      - "10000-19999:10000-19999"   # 工作区 IDE 端口范围（openvscode-server）
      - "20000-29999:20000-29999"   # 工作区测试环境端口范围
    environment:
    ...
    volumes:
      - /var/run/docker.sock:/var/run/docker.sock   # Docker Socket 挂载（工作区容器管理）
      - workspace_data:/data/workspaces              # 工作区持久化数据卷根目录
    depends_on:
      - db
      - redis
  ...
  # 工作区容器由 Teax 通过 Docker Socket 动态创建，不在 compose 中声明
...
| `createWorkspaceBodySchema` | workspace.dto.ts | 创建工作区（`name`, `branch`, `resources?`） |
...
| **工作区** | 容器化开发环境，Web VSCode + 测试环境网关，生命周期与工作区同步 |
...
6. **Phase 6（Week 11-13）**：工作区功能
   ...
   - **Week 12**：网关代理（Nitro server route `/workspace/[name]/[...path]`，支持 WebSocket）、工作区镜像构建（Dockerfile.workspace + entrypoint.sh）、端口分配策略
   - **Week 13**：前端页面（Workspaces Tab + 创建弹窗 + Admin 全局管理页）、workspace:create/workspace:manage 权限集成、审计日志集成

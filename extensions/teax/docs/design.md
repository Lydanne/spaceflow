...
> **Tab 路由结构**：每个 Tab 对应独立子路由路径（如 `/{orgName}/{projectName}/actions`），完全对标 Gitea 的 `/{owner}/{repo}/actions` URL 风格。`[projectName].vue` 作为布局页提供项目头部和 Tab 导航，子路由通过 `<NuxtPage>` 渲染。Tab 切换使用 `<NuxtLink>` 导航，天然支持刷新保持、链接分享、浏览器前进/后退。
...
│       └── /feishu                  # 飞书 OAuth 回调
│
├── /:orgName                        # 组织项目列表（类似 Gitea /:org 风格）
│   └── /:projectName                # 项目工作台（布局页：头部 + Tab 导航）
│       ├── /                        # README（默认子路由）
...
│
├── /org/:orgName                    # 组织管理（对标 Gitea /org/:org/settings）
│   ├── /settings                    # 组织设置（团队管理 + 权限组管理，仅 owner/admin）
│   └── /new                         # 创建项目（仓库搜索选择）
│
├── /account                         # 个人账号（待实现）
│   ├── /profile                     # 个人信息
...
> **路由冲突处理**：`/:orgName` 是动态 catch-all 路由，Nuxt 文件路由约定中固定路由（`/auth`、`/org`、`/-`）优先于动态路由匹配。系统管理使用 `/-/admin` 前缀，组织管理使用 `/org/:orgName` 前缀（对标 Gitea 的 `/org/:org/settings`），均不会与 `/:orgName/:projectName` 冲突。前端通过 `/api/resolve/:orgName/:projectName` API 将可读名称解析为 UUID，后端 API 保持使用 UUID 参数不变。

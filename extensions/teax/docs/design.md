...
| Tab | 功能描述 |
|-----|---------|
| **Actions** | 发布任务（PublishTask）管理，同时展示 Gitea Actions 运行记录 |
| **Agents** | Agent 实例管理，Session 运行状态监控 |
| **Pages** | 静态页面托管配置 |
| **小程序开发码管理** | 小程序预览码、体验码管理 |
| **...** | 更多扩展功能 |

> **Actions Tab 说明**：Teax 的发布系统（`PublishTask`）由 Gitea Webhook 触发或手动触发，与 Gitea 内置 Actions（`.gitea/workflows/*.yml`）独立运行。Actions Tab 同时展示两侧数据：Teax 发布任务状态 + 调用 Gitea API 获取的 workflow 运行记录。两者不互相替代。
...
1. **Phase 1（Week 1-2）**：Nuxt 4 项目初始化、数据库 Schema、Gitea OAuth 登录、组织/团队同步
2. **Phase 2（Week 3-4）**：项目管理、Gitea Webhook（含 HMAC 签名验证）、发布流程、基础权限中间件

...
Teax 扩展路由（Gitea 无，采用 GitHub 风格）：
  /:owner/:repo/agents             — Agent Session 管理（类似 GitHub Actions 子页面风格）
...
**直接采用 Gitea API v1 的规范**，路径使用 name-based（`owner/repo`）而非 UUID：
...
Gitea API v1 规范：
  ...
  /api/v1/user/repos                        — 当前用户的仓库列表
  /api/v1/orgs/{orgname}                    — 组织信息
  /api/v1/orgs/{orgname}/repos              — 组织仓库列表
  /api/v1/orgs/{orgname}/teams              — 组织团队列表
  /api/v1/orgs/{orgname}/members            — 组织成员列表
  /api/v1/repos/{owner}/{repo}              — 仓库信息
  /api/v1/repos/{owner}/{repo}/branches     — 分支列表
  /api/v1/repos/{owner}/{repo}/hooks        — Webhook 管理
  /api/v1/repos/{owner}/{repo}/actions/runs — Workflow Runs
  /api/v1/admin/users                       — 管理员：用户列表
  /api/v1/admin/orgs                        — 管理员：组织列表
```

**Teax API 采用相同结构**（前缀简化为 `/api/`，省略 `/v1`，因 Teax 为单版本应用）：

```text
Teax API 路径（= Gitea 风格 name-based）：

用户：
  GET    /api/user                              — 当前用户信息
  GET    /api/user/orgs                         — 当前用户的组织列表

组织：
  GET    /api/orgs/{orgname}                    — 组织信息
  GET    /api/orgs/{orgname}/repos              — 组织项目列表
  POST   /api/orgs/{orgname}/repos              — 创建项目
  GET    /api/orgs/{orgname}/teams              — 团队列表
  GET    /api/orgs/{orgname}/members            — 成员列表
  POST   /api/orgs/{orgname}/sync               — 同步组织（Teax 扩展）
  GET    /api/orgs/{orgname}/permissions         — 权限组列表（Teax 扩展）
  POST   /api/orgs/{orgname}/permissions         — 创建权限组（Teax 扩展）
  PUT    /api/orgs/{orgname}/permissions/{id}    — 更新权限组（Teax 扩展）
  DELETE /api/orgs/{orgname}/permissions/{id}    — 删除权限组（Teax 扩展）

项目/仓库（name-based，与 Gitea /repos/{owner}/{repo} 一致）：
  GET    /api/repos/{owner}/{repo}              — 项目详情
  DELETE /api/repos/{owner}/{repo}              — 删除项目
  PATCH  /api/repos/{owner}/{repo}/settings     — 更新项目设置
  GET    /api/repos/{owner}/{repo}/branches     — 分支列表
  GET    /api/repos/{owner}/{repo}/actions/runs       — Workflow Runs 列表
  GET    /api/repos/{owner}/{repo}/actions/workflows  — Workflow 定义列表
  POST   /api/repos/{owner}/{repo}/actions/workflows/{id}/dispatches — 触发 Workflow
  GET    /api/repos/{owner}/{repo}/agents             — Agent 列表（Teax 扩展）
  POST   /api/repos/{owner}/{repo}/agents             — 创建 Agent（Teax 扩展）
  GET    /api/repos/{owner}/{repo}/agents/{id}/sessions — Agent Sessions（Teax 扩展）
  GET    /api/repos/{owner}/{repo}/pages              — Pages 配置（Teax 扩展）
...
管理员：
  ...
  PATCH  /api/admin/users/{username}            — 更新用户（如切换管理员）
  GET    /api/admin/orgs                        — 组织列表
  GET    /api/admin/orgs/{orgname}              — 组织详情
  POST   /api/admin/orgs/{orgname}/sync         — 同步组织
  GET    /api/admin/orgs/{orgname}/teams         — 团队列表
  GET    /api/admin/audit-logs                  — 审计日志（Teax 扩展）
...
Webhook 接收：
  POST   /api/webhooks/gitea                    — Gitea Webhook 接收
```
...
| **路径参数** | name-based（`{owner}/{repo}`、`{orgname}`） | 完全采用，不再使用 UUID 路径 |
...
> **Tab 路由结构**：每个 Tab 对应独立子路由路径（如 `/{owner}/{repo}/actions`），采用 Gitea 的 `/{owner}/{repo}/*` URL 结构。`[repo].vue` 作为布局页提供项目头部和 Tab 导航，子路由通过 `<NuxtPage>` 渲染。Tab 切换使用 `<NuxtLink>` 导航，天然支持刷新保持、链接分享、浏览器前进/后退。
...
│       └── /feishu                  # 飞书 OAuth 回调
│
├── /:owner/:repo                    # 项目工作台（= Gitea /:owner/:repo）
│   ├── /                            # README / 概览（默认）
...
│
├── /org/:orgname                    # 组织管理（= Gitea /org/:orgname）
│   ├── /settings                    # 组织设置（团队 + 权限组管理）
│   └── /new                         # 创建项目（Teax 扩展，Gitea 用 /repo/create）
│
├── /user/settings                   # 用户设置（= Gitea /user/settings）
│   ├── /profile                     # 个人信息
│   ├── /feishu                      # 飞书绑定（Teax 扩展）
│   └── /notifications               # 通知设置
│
...
> 前端页面通过 `/api/repos/{owner}/{repo}` 等 name-based API 获取数据，后端内部使用 UUID 做数据库查询。
...
│   ├── [owner]/               # /:owner — 组织项目列表
│   │   ├── index.vue
│   │   ├── [repo].vue         # /:owner/:repo — 项目布局页（Tab 导航）
│   │   └── [repo]/            # 项目子路由
│   │       ├── index.vue      # README / 概览
...
│   │       └── settings.vue   # 项目设置
│   ├── org/                   # /org/:orgname（= Gitea）
│   │   └── [orgname]/
│   │       ├── settings.vue   # 组织设置
│   │       └── new.vue        # 创建项目
│   ├── user/                  # /user/settings（= Gitea）
│   │   └── settings/
│   │       ├── profile.vue
│   │       ├── feishu.vue
│   │       └── notifications.vue
...
API 路由采用 Gitea name-based 路径风格（`repos/[owner]/[repo]` 而非 UUID）：

```text
server/
├── api/                       # API 路由（= Gitea API 风格）
│   ├── auth/                  # /api/auth/*
...
│   ├── user/                  # /api/user/*（= Gitea /api/v1/user）
│   │   ├── index.get.ts           # 当前用户信息
│   │   └── orgs.get.ts            # 当前用户的组织列表
│   ├── orgs/                  # /api/orgs/{orgname}/*（= Gitea /api/v1/orgs）
│   │   └── [orgname]/
│   │       ├── index.get.ts       # 组织信息
│   │       ├── repos.get.ts       # 组织项目列表
│   │       ├── repos.post.ts      # 创建项目
│   │       ├── teams.get.ts       # 团队列表
│   │       ├── members.get.ts     # 成员列表
│   │       ├── sync.post.ts       # 同步组织（Teax 扩展）
│   │       └── permissions/       # 权限组管理（Teax 扩展）
│   │           ├── index.get.ts
│   │           ├── index.post.ts
│   │           ├── [id].put.ts
│   │           └── [id].delete.ts
│   ├── repos/                 # /api/repos/{owner}/{repo}/*（= Gitea /api/v1/repos）
│   │   ├── search.get.ts         # 搜索仓库
│   │   └── [owner]/[repo]/
│   │       ├── index.get.ts       # 项目详情
...
│   │       └── actions/
│   │           ├── runs.get.ts    # Workflow Runs 列表
│   │           ├── workflows.get.ts  # Workflow 定义列表
│   │           └── workflows/[id]/dispatches.post.ts  # 触发 Workflow
│   ├── admin/                 # /api/admin/*（= Gitea /api/v1/admin）
│   │   ├── users/
│   │   │   ├── index.get.ts       # 用户列表
│   │   │   └── [username].patch.ts  # 更新用户
│   │   ├── orgs/
│   │   │   ├── index.get.ts       # 组织列表
│   │   │   └── [orgname]/
│   │   │       ├── index.get.ts   # 组织详情
│   │   │       ├── sync.post.ts   # 同步组织
│   │   │       └── teams.get.ts   # 团队列表
│   │   └── audit-logs.get.ts      # 审计日志（Teax 扩展）
│   └── webhooks/
│       └── gitea.post.ts          # Gitea Webhook 接收
├── middleware/                # 服务端中间件
...
├── jobs/                      # BullMQ 任务
└── utils/                     # 工具函数
    ├── gitea.ts               # GiteaService（Gitea API v1 封装）
    ├── feishu.ts              # 飞书 API 封装
...
> Teax 后端通过 Gitea API v1 访问 Gitea 服务。Teax 自身 API 的路径结构直接采用 Gitea API 的 name-based 风格（详见「设计规范 > 后端 API 规范」）。
...
| `POST /api/v1/users/{username}/tokens` | 创建 PAT | Service Token 管理 |

...
> **Tab 路由结构**：每个 Tab 对应独立子路由路径（如 `/:projectId/actions`），类似 Gitea 的 `/{owner}/{repo}/actions` 风格。`[projectId].vue` 作为布局页提供项目头部和 Tab 导航，子路由通过 `<NuxtPage>` 渲染。Tab 切换使用 `<NuxtLink>` 导航，天然支持刷新保持、链接分享、浏览器前进/后退。
...
│       ├── /projects                # 项目列表
│       │   ├── /new                 # 创建项目（仓库搜索选择）
│       │   └── /:projectId          # 项目工作台（布局页：头部 + Tab 导航）
│       │       ├── /                # README（默认子路由）
│       │       ├── /actions         # Actions（Gitea workflow runs）
│       │       ├── /agents          # Agents（Phase 3）
│       │       ├── /pages           # Pages（Phase 3）
│       │       └── /settings        # 设置（基本信息 + 通知设置 + 删除项目，仅 owner/admin）
│       │

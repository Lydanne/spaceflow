...
- [x] 飞书消息通知（构建成功/失败 + Push 事件 + Agent 结果通知）
- [x] 飞书机器人交互（/help /status /deploy /rollback /list，含权限校验）
- [x] 飞书审批流程（审批请求创建 + 飞书实例同步 + 状态回调 + 结果通知）
- [x] 用户飞书绑定管理页面（/user/settings）
- [x] 通知偏好设置（publish/approval/agent/system 四种类型）

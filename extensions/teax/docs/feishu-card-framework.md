...
5. **渐进迁移**：兼容现有 `FeishuCardBuilder` 和手写 JSON，可逐步迁移
...
│   ├── builder.ts                    # 增强版 CardBuilder
│   └── define.ts                     # defineCardPage 工具函数
│
├── card-pages/                       # 卡片页面（按业务分目录）
│   ├── control-panel/
│   │   ├── home.page.ts              # cp:home
│   │   ├── org.page.ts              # cp:org
│   │   ├── repo.page.ts            # cp:repo
│   │   └── actions.page.ts         # cp:actions
│   ├── account/
│   │   └── index.page.ts           # account:home
│   ├── preset-console/
│   │   ├── form.page.ts            # preset:form
│   │   └── result.page.ts          # preset:result
│   └── test/
│       └── form.page.ts            # test:form
│
└── services/
    └── bot-command.service.ts        # handleCardAction 简化为一行路由分发
```
...
在现有 `FeishuCardBuilder` 基础上扩展。CardKit 的 `EnhancedCardBuilder` 强制使用 **JSON 2.0 schema**，按钮直接作为 body element（不用 `action` 容器包裹），`value` 为对象类型。按钮支持 `navigate` / `action` / `url` 三种模式：
...
CardRouter 的 `dispatch()` 是**可选的 fallback**。在 `handleCardAction` 中，先走 CardRouter，如果没有匹配的 page，再 fallback 到现有的 if-else 逻辑：

```typescript
export async function handleCardAction(ctx: CardActionContext) {
  // 1. 尝试 CardKit 路由
  const result = await cardRouter.dispatch(ctx);
  if (result !== undefined) {
    return result;
  }

  // 2. Fallback: 现有 if-else 分发（逐步迁移后删除）
  // ... 现有 control-panel / account / preset-console 逻辑 ...
}
```

#### 3. 现有 CardStateMachine 复用

对于需要 Redis 状态的多步骤流程（如审批），CardPage 可以内部使用 `CardStateMachine`：
...
defineCardPage({
  ...
  async render(ctx) {
    // 使用 StateMachine 管理多步骤
    const machine = getStateMachine("approval");
    const { card } = await machine.start({ ... });
    return card;
  },

  async onAction(ctx) {
    const machine = getStateMachine("approval");
    await machine.handleAction({ token: ctx.params.token, action: ctx.action });
  },
});
```
...
| `preset` | `preset:form` | 预设控制台表单 |
| `preset` | `preset:result` | 预设触发结果 |
| `test` | `test:form` | 测试表单 |
| `test` | `test:result` | 测试表单提交结果 |
| `approval` | `approval:detail` | 审批详情 |
...
### Phase 1：核心框架（约 300 行）

| 文件 | 说明 | 估计行数 |
| --- | --- | --- |
| `server/card-kit/types.ts` | 类型定义 | ~80 |
| `server/card-kit/define.ts` | `defineCardPage()` + `navigate()` + `toast()` | ~30 |
| `server/card-kit/router.ts` | `CardRouter` — 注册 + 分发 | ~100 |
| `server/card-kit/builder.ts` | `card()` — EnhancedCardBuilder | ~70 |
| `server/card-kit/index.ts` | 统一导出 | ~10 |

### Phase 2：迁移第一个页面

将 `test:form` 从 `bot-command.service.ts` 迁移到 `server/card-pages/test/form.page.ts`，验证框架可行性。

### Phase 3：迁移 control-panel

将 `control-panel.service.ts` 的 5 个页面（home / org / repo / actions / settings）迁移到 `server/card-pages/control-panel/`。

### Phase 4：迁移 account + preset-console

迁移剩余的卡片交互。

### Phase 5：清理

删除 `handleCardAction` 中的旧 if-else 分发逻辑，全部走 CardRouter。
...
### Q: 与现有 approval-flow 的 card-handler 如何共存？

approval-flow 使用 `action.value` 字符串格式（如 `"approval_flow:approve:xxx"`），不包含 `__page` 字段。CardRouter 在 dispatch 时检测到没有 `__page` 就返回 `undefined`，fallback 到现有逻辑。

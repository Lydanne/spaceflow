...
4. **渐进迁移**：兼容现有 `FeishuCardBuilder` 和手写 JSON，可逐步迁移
...
在现有 `FeishuCardBuilder` 基础上扩展，按钮支持 `navigate` / `action` / `url` 三种模式：
...
export default defineCardPage({
  ...
  async render(ctx) {
    return ctx.card({ title: "🧪 表单组件测试", theme: "blue", schema: "2.0" })
      .text("此卡片使用 **JSON 2.0** 结构，演示 `form` 表单容器。")
      .divider()
      .form("test_form")
        .inputV2({
          name: "test_username",

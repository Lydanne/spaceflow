...
- **数据展示**：Teax 后端通过 Gitea API（`/api/v1/repos/{owner}/{repo}/actions/runs`）获取 workflow runs，前端通过 `/api/repos/{owner}/{repo}/actions` 代理访问
      ...
      - name: AI Review PR
        run: |
          SESSION_ID=$(curl -s -X POST \
            ...
            https://teax.example.com/api/repos/${{ github.repository }}/agents/sessions | jq -r '.id')
          ...
          while true; do
            STATUS=$(curl -s \
              -H "Authorization: token ${{ secrets.TEAX_TOKEN }}" \
              https://teax.example.com/api/repos/${{ github.repository }}/agents/sessions/$SESSION_ID | jq -r '.status')
...
详见 [Agents 当前实现文档](../design/agent-system.md) 与 [Agents Runtime 文档](./agents-runtime.md)。
...
- **耗时**：空值自动隐藏（不显示无意义的 "-"）
- 点击整个卡片跳转到 Run 详情页（`/:owner/:repo/actions/runs/:runId`）查看 Jobs 列表和日志
...
路由：`/:owner/:repo/actions/runs/:runId`
...
- [Overview 导航](./index.md) - 使用文档入口
- [架构概览](../design/architecture.md) - 系统整体架构
- [API 规范](../design/api-specification.md) - API 路径设计

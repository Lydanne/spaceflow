...
import {
  buildAutoApproveBody,
  buildLineReviewBody,
  deleteOldReviews,
  issueToReviewComment,
  lineMatchesPosition,
} from "./review-result-model.helpers";
...
export class ReviewResultModel {
  ...
  private buildAutoApproveBody(stats: ReviewStats, prAuthorLogin?: string): string {
    return buildAutoApproveBody(stats, prAuthorLogin);
  }
  ...
  async deleteOldReviews(): Promise<void> {
    const deletedCount = await deleteOldReviews(this.pr);
    if (deletedCount > 0) {
  ...
  lineMatchesPosition(issueLine: string, position?: number): boolean {
    return lineMatchesPosition(
      issueLine,
      position,
      this.deps.reviewSpecService.parseLineRange.bind(this.deps.reviewSpecService),
    );
  }
  ...
  buildLineReviewBody(issues: ReviewIssue[], round: number, allIssues: ReviewIssue[]): string {
    return buildLineReviewBody(issues, round, allIssues, ReviewResultModel.buildRoundTitle);
  }
}
